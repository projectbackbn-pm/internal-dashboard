    ...
    <aside style={{
      ...
      <div style={{ padding: "22px 20px 18px", borderBottom: `1px solid ${C.border}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 38, height: 38, borderRadius: 11, background: C.purple, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ padding: "14px 16px", borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: "50%", background: C.purpleLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: C.purple }}>FP</div>
          <div>
...
function KPICard({ label, value, sub, positive, Icon, iconBg, iconColor }) {
  return (
    <div style={{ flex: 1, minWidth: 140, background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, padding: "22px", boxShadow: C.shadow }}>
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      <thead>
        <tr>
          {heads.map((h) => (
            <th key={h} style={{ padding: "12px 20px", textAlign: "left", color: C.textMuted, fontWeight: 500, fontSize: 12, borderBottom: `1px solid ${C.border}` }}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
...
function ClientsView({ clients, onNav }) {
  ...
  let filtered = clients.filter((c) => `${c.name}${c.company}${c.email}`.toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <div style={{ padding: "28px" }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: C.text }}>Todos los clientes</h1>
        ...
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 200, position: "relative" }}>
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar cliente..."
              style={{ width: "100%", boxSizing: "border-box", background: "#F4F6FB", border: `1px solid ${C.border}`, borderRadius: 10, padding: "9px 12px 9px 34px", fontSize: 13, color: C.text, outline: "none" }} />
          </div>
...
const inp = { width: "100%", boxSizing: "border-box", background: "#F4F6FB", border: `1px solid #E5E7EB`, borderRadius: 10, padding: "10px 14px", color: "#111827", fontSize: 13, outline: "none", fontFamily: "inherit" };
...
function ClientForm({ client, isNew, onSave, onDelete, onBack }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: C.bg }}>
      <div style={{ background: C.white, borderBottom: `1px solid ${C.border}`, padding: "15px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
      ...
      <div style={{ padding: "26px 28px", maxWidth: 820 }}>
        <div style={{ display: "flex", gap: 12, marginBottom: 22, flexWrap: "wrap" }}>
          {[
            ...
            <div key={x.label} style={{ flex: 1, minWidth: 140, background: C.white, border: `1px solid ${C.border}`, borderRadius: 12, padding: "14px 16px", boxShadow: C.shadow }}>
        ...
        <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, boxShadow: C.shadow, overflow: "hidden" }}>
          <div style={{ display: "flex", borderBottom: `1px solid ${C.border}`, padding: "0 22px" }}>
            {tabs.map((t) => (
              <button key={t.id} onClick={() => setTab(t.id)} style={{ background: "transparent", border: "none", borderBottom: tab === t.id ? `2px solid ${C.purple}` : "2px solid transparent", color: tab === t.id ? C.purple : C.textSub, padding: "14px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", marginBottom: -1 }}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          <div style={{ padding: "24px 22px" }}>
            {tab === "info" && (
              <div>
                ...
                <Fld label="Notas"><textarea value={form.notes} onChange={(e) => u("notes", e.target.value)} rows={4} placeholder="Observaciones, contexto del cliente..." style={{ ...inp, resize: "vertical" }} /></Fld>
                {!isNew && (
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("finpanel_v2");
  ...
  useEffect(() => {
    if (!loaded) return;
    window.storage.set("finpanel_v2", JSON.stringify(clients)).catch(() => {});
  }, [clients, loaded]);

  const nav = (v, id = null) => { setView(v); if (v === "client-form") setEditId(id); };
  const saveClient = (data) => {
    if (!editId) { setClients((p) => [...p, { ...blankClient(), ...data }]); }
    else         { setClients((p) => p.map((c) => (c.id === editId ? { ...c, ...data } : c))); }
    nav("clients");
  };
  ...
  return (
    <div style={{ fontFamily: "system-ui,-apple-system,sans-serif", background: C.bg, color: C.text, minHeight: "100vh", display: "flex" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        {view !== "client-form" && (
          <div style={{ background: C.white, borderBottom: `1px solid ${C.border}`, padding: "14px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 1px 2px rgba(0,0,0,0.04)" }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: C.text }}>
              {{ dashboard: "Bienvenido 👋", clients: "Clientes" }[view] || ""}
            </div>
            <div style={{ position: "relative" }}>
              <Search size={14} color={C.textMuted} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)" }} />
              <input placeholder="Buscar..." style={{ background: "#F3F4F6", border: `1px solid ${C.border}`, borderRadius: 10, padding: "9px 14px 9px 34px", fontSize: 13, color: C.text, outline: "none", width: 220 }} />
            </div>
          </div>
        )}
